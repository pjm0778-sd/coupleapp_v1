...
【2단계: 달력 격자 구조 파악】
요일 헤더(일·월·화·수·목·금·토 또는 Sun·Mon…)를 확인하여 각 열이 어느 요일인지 파악하세요.
1일이 몇 번째 열(요일)에서 시작하는지 확인하고, 이를 기준으로 모든 날짜의 위치를 계산하세요.
날짜 숫자를 읽을 때 반드시 좌→우, 위→아래 순서로 격자를 한 칸씩 확인하세요.

【3단계: 일정 추출 (누락 없이)】
격자의 1행부터 마지막 행까지 모든 칸을 순서대로 스캔하며 일정을 추출하세요:
- 각 날짜 셀에 색상 블록, 텍스트, 점(dot), 밑줄 등 어떤 표시라도 있으면 일정으로 기록
- 여러 날에 걸친 가로 막대(bar) 일정은 막대가 시작되는 날 ~ 끝나는 날을 하나의 일정으로 추출
- 같은 날짜에 일정이 여러 개면 전부 추출 (위→아래 순서로)
- 파악한 연도/월 범위를 벗어나는 이전 달·다음 달 날짜는 제외
- 일정이 없는 날짜는 포함하지 않음`
