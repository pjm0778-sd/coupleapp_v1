...
class NotificationManager {
  ...
  /// Android 13+ / iOS 알림 권한 요청. true = 허용됨
  ...
  Future<void> notifyScheduleChanged({
    ...
    final actionLabel = switch (type) {
      NotificationType.scheduleAdded => '추가했어요',
      NotificationType.scheduleDeleted => '삭제했어요',
      NotificationType.scheduleUpdated => '수정했어요',
      _ => '변경했어요',
    };
    ...
    await showLocalNotification(
      ...
      title: '📅 파트너 일정 알림',
      body: '파트너가 "$scheduleTitle" 일정을 $actionLabel',
      type: type,
    );
  }
}
